...
#Establecer ID unico a cada ingreso
...
#Eliminar ingresos
def eliminarins(df,n):
    random_indices = np.random.choice(df.index, size=n, replace=False)
    # Asignar NaN a esos registros en la columna 'fecha'
    df.loc[random_indices, 'fecha'] = np.nan
    return df

#Graficar duraciones del GT
def graficarGT(tabla):
    fig = px.line(tabla, x='duracion(m)', y='n_persona', title='Duración vs Personas')
...
def main(df2,dia,n,n_ins,metodo,intervalo_mananero,intervalo_tarde,proporciones,intervalos,table_name):
    df2['id_cc']=1
    ...
    for fecha, grupo in grupos_por_dia:
        ...
        print(df)        
        #Eliminar ingresos
        data=eliminarins(df,n_ins)
        #------------------------
        df1,tabla=getdata(data,fecha) #
        df2=insouts(df1)
        df2['id_cc']=1
...
n_days=5000 #Numero de dias a eliminae
n=0 #Numero de dia a procesar, se usa si son dias unicos
table_name='insouts6' #en que tabla de insouts guardamos

main(df2,dia,n,n_days,funsalidas,intervalo_mananero,intervalo_tarde,proporciones,intervalos,table_name)